...
// Function to open profile popup - can be called from any page
function openProfilePopup(userId) {
    ...
    popup.id = 'profilePopupContainer';
    document.body.appendChild(popup);
    
    // Load the popup content
    fetch('popup-profile.html')
        .then(response => response.text())
        .then(html => {
            popup.innerHTML = html;
            
            // Now populate the popup with user data
            document.getElementById('popupProfileName').textContent = userData.lastName + ', ' + userData.firstName;
            document.getElementById('popupProfileEmail').textContent = userData.email;
            document.getElementById('popupProfileType').textContent = '[' + userData.type + ']';
            document.getElementById('popupProfileDepartment').textContent = userData.department;
            document.getElementById('popupProfileBio').textContent = userData.biography;
            
            if (userData.image) {
                document.getElementById('popupProfileImage').src = userData.image;
            }
            
            // Setup close button functionality
            document.getElementById('closeProfilePopup').addEventListener('click', closeProfilePopup);
            document.getElementById('profilePopupOverlay').addEventListener('click', closeProfilePopup);
        })
        .catch(error => {
            console.error('Error loading profile popup:', error);
        });
}

function closeProfilePopup() {
    const popupContainer = document.getElementById('profilePopupContainer');
    if (popupContainer) {
